...
export function buildTerminalStartupProbeCommand(agent: AgentId, workspaceRoot: string, markerPath: string): string {
  ...
  return [
    ...
    "if ($__hydraStartupProbeDir) { New-Item -ItemType Directory -LiteralPath $__hydraStartupProbeDir -Force | Out-Null }",
...
export function buildPowerShellDispatchCommand(
  ...
  return [
    ...
    "if ($__hydraReplyDir) { New-Item -ItemType Directory -LiteralPath $__hydraReplyDir -Force | Out-Null }",
    ...
    "if ($__hydraLogDir) { New-Item -ItemType Directory -LiteralPath $__hydraLogDir -Force | Out-Null }",
    "$__hydraUtf8NoBom = [System.Text.UTF8Encoding]::new($false)",
