...
Terminal prompts, launchers, replies, logs, and session snapshots live under a workspace-specific directory rooted in VS Code `ExtensionContext.storageUri`. A hashed directory under `globalStorageUri` is used when workspace storage is unavailable. The bridge rejects roots inside the workspace, checks containment and file identity, uses private permissions where the platform supports them, authenticates replies and log snapshots, and deletes request material promptly. A reply key is a create-new raw 32-byte file consumed and deleted by PowerShell before native command resolution; the pasted command and child environment do not carry it. Request names record the owning extension-host PID. Each bridge sweeps periodically and at lifecycle boundaries. Early deletion of a fresh key requires proof that its owner PID is dead, so live, ambiguous, and reused PIDs are not cross-window cleanup targets; independently, the existing one-hour age rule remains the fallback for every eligible leftover, including legacy names.

This boundary prevents ambient key disclosure through terminal history, environment inheritance, and ordinary workspace files. It does not isolate secrets from pre-existing or detached processes, debuggers, shell profiles, administrators, or other code with the same OS-user authority. Full Native execution can create such code, so same-user compromise is terminal-bridge compromise; the safe response is to stop the bridge and affected terminals and restart the extension host.
