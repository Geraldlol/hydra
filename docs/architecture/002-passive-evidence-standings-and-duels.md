...
The automatic path is narrower still. Before a serial builder receives write access, Hydra resolves each eligible package-manager executable to an absolute path outside the workspace and latches the exact verification command. Only a bounded package-script grammar is scoring-eligible; arbitrary or dynamic shell commands may still run as ordinary verification but append no standings evidence. Hydra also fingerprints a bounded conventional verifier-control inventory containing package manifests and lockfiles, package-manager runtime configuration and executable scripts, tests/specs, scripts, and common test/build/lint configuration. The same inventory must match before dispatch, immediately before verification, and after verification. This is a conservative integrity guard, not a claim that arbitrary shell dependency closure is knowable or that the tests are complete or independent.

A successful serial builder is eligible only when that pre-dispatch plan remains bound to the executed receipt, the bounded verifier controls remain unchanged, the Git-visible workspace changes during its turn, Hydra's automatic post-build verification passes cleanly, and a second fingerprint proves the verification command left that exact post-build state intact. The generated claim binds the verification receipt, command-plan hash, verifier-control hash, and post-build workspace hash. Unique receipts retain unique claims for audit, but receipts under the same plan/control regime share one correlated round; repeated trivial edits therefore cannot manufacture independent evidence maturity. A new plan or verifier-control hash begins a new automatic evidence round.

The claim says only that the recorded state following that head's turn passed the recorded command; it does not claim authorship of every concurrent change or that the implementation is globally correct. The bounded snapshots do not defend against a malicious same-user process that changes and restores controls between captures. No-op builds, parallel builds with ambiguous attribution, unavailable fingerprints, unsupported or changed verifier controls, failed/cancelled/timed-out or unconfirmed verification, and state-changing verification runs append nothing. Receipt-derived event IDs are deterministic, so retries and concurrent windows are idempotent. `hydraRoom.autoScorePassingBuilds` defaults on and can be disabled in User Settings; like every boolean automatic control, it is application-scoped and trust-restricted.
...
1. Only a successful serial reactor or closer reply may initiate a challenge. In a prompt carrying the duel protocol, top-level `Challenge:` is reserved for a formal commitment; ordinary disagreement uses `Amend:`. The head emits Hydra's strict challenge control record immediately before its Decision Packet and names the head whose reply it just examined. The strict protocol follows the generic phase prose so the machine contract wins any wording ambiguity. A top-level challenge with a complete normal Decision Packet but no control record produces an explicit protocol rejection and no Elo rather than failing silently; Hydra never invents proposition, domain, evidence, or rationale from prose. Hydra binds the challenger from the actual reply role and the opponent/source from the current turn; an exact/minimal reply, human prompt, free-form disagreement, forged opponent ID, stale message, opener reply, quoted marker, or parallel reply cannot create a duel.
...
- Changed serial builds now generate evidence without a manual recorder step, while pre-dispatch command/control latching plus strict fingerprint and receipt binding keeps self-redefined tests and passing commands from becoming broad correctness claims.
- Cross-window ledger watching keeps passive standings fresh without making disposable mirrors authoritative.
...
Validation covers event-shape and reference checks, required evidence provenance, automatic changed-serial-build eligibility, pre-dispatch verifier-plan latching, bounded control-surface changes and unsupported-command rejection, pre/post/verified fingerprint equality, deterministic retry IDs, cross-process idempotent append, serialized cross-window fail-closed refresh, nonfatal append failures, actor-attributed append-only reversal semantics, active-evidence inspection, peer weight zero, source weighting, per-round evidence caps, Wilson small-sample behavior, and the five-round provisional boundary. Duel coverage includes strict reactor/closer marker parsing, explicit prompt provenance, prompt ordering, missing-marker diagnostics, quote/fence and forged-opponent rejection, source-turn binding, host-built evidence bounds, atomic challenge/admission appends, no human creation surface, no exhibition fallback, persistent-consent and equal-capability preflight rejection, exact command/model/args/cwd/environment-digest locking, bounded project-content and entry-metadata fingerprinting, live mutation monitoring and cancellation, one-active-duel/cooldown/daily-cap enforcement, automatic two-head sealing, successful-cycle paired reveal, strict head-response identity binding, execution-receipt hashing, redacted call tracing, cancellation and orphan-secret cleanup races, Elo replay and zero-sum deltas, exact-zero ties, concurrent append serialization, fail-closed ledger loading, and one-way Markdown generation. UI and contract tests must keep agent initiation, human adjudication, equal maximum granted permissions without catalog-identity claims, read-only project integrity and its same-user limitation, no-authority warnings, and the receipt limitation visible.
