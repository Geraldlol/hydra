...
To keep long rooms from sending the same old discussion back to both agents on every turn, prompt injection keeps only the newest active transcript messages once `hydraRoom.promptTranscriptMaxChars` is exceeded. The default is `400000` characters: a per-call cost and attention guardrail, not a model context-window limit. When the active transcript crosses that cap, Hydra also runs a wiki context refresh so durable facts can be compacted into `.hydra/wiki/context.md` before older transcript content is omitted. The omitted history remains in `.hydra/transcript.md` or `.hydra/archive/`.
